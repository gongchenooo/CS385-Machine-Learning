...
class LogisticRegression(object):
    ...
    def loss(self, y_predict, y_true, class_id):
        ...
        :return:
        '''
        # mean_i[y_i*x_i*beta - log(1+exp(x_i*beta))] + 0.5 * lambda * |beta|^2
        return -np.mean(y_true * y_predict - np.log(1 + np.exp(y_predict))) + 0.5 * self.reg_lam * np.linalg.norm(self.beta_list[class_id])**2
    ...
    def gradient(self, x, y_true, y_predict, class_id):
        ...
        return np.mean(np.matmul(x, y_predict-y_true), axis=0) + self.reg_lam * self.beta_list[class_id]
