...
class LogisticRegression(object):
    ...
    def loss(self, y_predict, y_true):
        ...
        :return:
        '''
        # negative cross entropy: mean_i[y_i*log(p_i)] + 0.5*lam*|beta|**2
        labels_true = np.where(y_true == 1)
        return -np.mean(np.log(y_predict[labels_true])) + 0.5 * self.reg_lam * np.linalg.norm(self.beta)**2
    ...
    def gradient(self, x, y_true, y_predict):
        ...
        return np.mean(np.matmul(x, y_predict-y_true), axis=0) + self.reg_lam * self.beta
    ...
    def train(self, x_train, y_train, x_test, y_test):
        ...
        for i in tqdm(range(self.epochs)):
            ...
            if (i%10 == 0):
                ...
                np.save("res_dic_{}.npy".format(self.reg_lam), res_dic)
