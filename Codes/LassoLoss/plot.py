...
lam_list = ['0.1', '0.01', '0.001', '0.0001', '1e-05']
res_dic = {}

def draw_test():
    for lam in lam_list:
        res_dic[lam] = np.load('Results/ten/res_dic_{}.npy'.format(lam), allow_pickle=True).item()['accuracy_test']
        x = range(len(res_dic[lam]))
    x = [i*10 for i in x]
    plt.clf()
    plt.xlabel('Epoch', fontsize=20)
    plt.ylabel('Testing Accuracy(Multi Classifier)', fontsize=20)
    for lam in lam_list:
    ...
    plt.yticks(size = 20)
    plt.tight_layout()
    plt.savefig('Testing Accuracy(Multi Classifier).png')
    plt.show()
#draw_test()
...
def draw_lambda():
    ...
    for lam in lam_list:
        tmp_l2 = np.load('Results/ten/res_dic_{}.npy'.format(lam), allow_pickle=True).item()['beta'][-1].reshape(-1)
    ...
    plt.ylabel('L1-Norm of Parameter', fontsize=20)
