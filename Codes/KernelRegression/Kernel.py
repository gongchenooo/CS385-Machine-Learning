...
class Kernels:
    ...
    def locguass(x1, x2, d=None, gamma=None):
        ...
        if not d:
            ...
            d = d
        if x1.ndim == 1 and x2.ndim == 1:
            return (np.maximum(0, 1 - gamma * np.linalg.norm(x1 - x2) / 3) ** d) * np.exp(
                -gamma * np.linalg.norm(x1 - x2) ** 2)
        elif (x1.ndim > 1 and x2.ndim == 1) or (x1.ndim == 1 and x2.ndim > 1):
            return (np.maximum(0, 1 - gamma * np.linalg.norm(x1 - x2, axis=1) / 3) ** d) * np.exp(
                -gamma * np.linalg.norm(x1 - x2, axis=1) ** 2)
        elif x1.ndim > 1 and x2.ndim > 1:
            return (np.maximum(0, 1 - gamma * np.linalg.norm(x1[:, np.newaxis] - x2[np.newaxis, :],
                                                             axis=2) / 3) ** d) * np.exp(
                -gamma * np.linalg.norm(x1[:, np.newaxis] - x2[np.newaxis, :], axis=2) ** 2)
    ...
    def etakernel(x1, x2, d=None, gamma=None, op=None):
        ...
        if op == 'eta':
            ...
            return Kernels.linrbf(x1, x2).dot(Kernels.rbfpoly(x1, x2)) + Kernels.rbfpoly(x1, x2).dot(
                Kernels.rbfcosine(x1, x2))
        elif op == 'etasig':
            return Kernels.sigmoid(x1, x2).dot(Kernels.rbf(x1, x2)) + Kernels.rbfpoly(x1, x2).dot(
                Kernels.sigmoid(x1, x2))
        elif op == 'etaalpha':
            return Kernels.rbf(Kernels.linear(x1, x2).dot(Kernels.rbfpoly(x1, x2)),
                               Kernels.sigmoid(x1, x2) + Kernels.polynomial(x1, x2))
    ...
    def alignment(x1, x2, d=None, gamma=None, op=None):
        ...
        kappa_lin = Kernels.linear(x1, x2) - 1 / len(x1) * np.ones((x1.shape[0], x1.shape[0])).dot(
            np.ones((x1.shape[0], x1.shape[0])).T).dot(Kernels.linear(x1, x2)) \
                    - 1 / len(x1) * Kernels.linear(x1, x2).dot(
            np.ones((x1.shape[0], x1.shape[0])).dot(np.ones((x1.shape[0], x1.shape[0])).T)) + \
                    1 / len(x1) ** 2 * (np.ones((x1.shape[0], x1.shape[0])).T.dot(Kernels.linear(x1, x2))).dot(
            np.ones((x1.shape[0], x1.shape[0]))).dot(
            np.ones((x1.shape[0], x1.shape[0])).dot(np.ones((x1.shape[0], x1.shape[0])).T))
        kappa_rbf = Kernels.rbf(x1, x2) - 1 / len(x1) * np.ones((x1.shape[0], x1.shape[0])).dot(
            np.ones((x1.shape[0], x1.shape[0])).T).dot(Kernels.rbf(x1, x2)) \
                    - 1 / len(x1) * Kernels.rbf(x1, x2).dot(
            np.ones((x1.shape[0], x1.shape[0])).dot(np.ones((x1.shape[0], x1.shape[0])).T)) + \
                    1 / len(x1) ** 2 * (np.ones((x1.shape[0], x1.shape[0])).T.dot(Kernels.rbf(x1, x2))).dot(
            np.ones((x1.shape[0], x1.shape[0]))).dot(
            np.ones((x1.shape[0], x1.shape[0])).dot(np.ones((x1.shape[0], x1.shape[0])).T))
        kappa_poly = Kernels.polynomial(x1, x2) - 1 / len(x1) * np.ones((x1.shape[0], x1.shape[0])).dot(
            np.ones((x1.shape[0], x1.shape[0])).T).dot(Kernels.polynomial(x1, x2)) \
                     - 1 / len(x1) * Kernels.polynomial(x1, x2).dot(
            np.ones((x1.shape[0], x1.shape[0])).dot(np.ones((x1.shape[0], x1.shape[0])).T)) + \
                     1 / len(x1) ** 2 * (np.ones((x1.shape[0], x1.shape[0])).T.dot(Kernels.polynomial(x1, x2))).dot(
            np.ones((x1.shape[0], x1.shape[0]))).dot(
            np.ones((x1.shape[0], x1.shape[0])).dot(np.ones((x1.shape[0], x1.shape[0])).T))
        kappa_sigmoid = Kernels.sigmoid(x1, x2) - 1 / len(x1) * np.ones((x1.shape[0], x1.shape[0])).dot(
            np.ones((x1.shape[0], x1.shape[0])).T).dot(Kernels.sigmoid(x1, x2)) \
                        - 1 / len(x1) * Kernels.sigmoid(x1, x2).dot(
            np.ones((x1.shape[0], x1.shape[0])).dot(np.ones((x1.shape[0], x1.shape[0])).T)) + \
                        1 / len(x1) ** 2 * (np.ones((x1.shape[0], x1.shape[0])).T.dot(Kernels.sigmoid(x1, x2))).dot(
            np.ones((x1.shape[0], x1.shape[0]))).dot(
            np.ones((x1.shape[0], x1.shape[0])).dot(np.ones((x1.shape[0], x1.shape[0])).T))
        if op == 'linrbf':
            return kappa_lin.dot(kappa_rbf) / np.sqrt(
                np.sum(kappa_lin.dot(kappa_lin)) * np.sum(kappa_rbf.dot(kappa_rbf)))
        elif op == 'rbfpoly':
            return kappa_rbf.dot(kappa_poly) / np.sqrt(
                np.sum(kappa_rbf.dot(kappa_rbf)) * np.sum(kappa_poly.dot(kappa_poly)))
        elif op == 'rbfsig':
            return kappa_rbf.dot(kappa_sigmoid) / np.sqrt(
                np.sum(kappa_rbf.dot(kappa_rbf)) * np.sum(kappa_sigmoid.dot(kappa_sigmoid)))
        elif op == 'polysig':
            return kappa_poly.dot(kappa_sigmoid) / np.sqrt(
                np.sum(kappa_poly.dot(kappa_poly)) * np.sum(kappa_sigmoid.dot(kappa_sigmoid)))
        return 