...
class KLR(EvalC, loss, Kernels):
    ...
    def cost(self):
        ...
        return -1 / len(self.Y) * (self.Y.T.dot(np.dot(self.knl, self.alpha)) - np.sum(
            np.log(1 + np.exp(np.dot(self.knl, self.alpha)))) - \
    ...
    def fit(self, X, y, lr: float = None, iterations: int = None):
        ...
        for ii in range(self.iterations):
            ...
            self.alpha = self.alpha - self.lr * (
                        self.knl.dot(self.Y) * (np.ones(self.X.shape[0]) - self.activation()) + self.lamda * np.dot(
                    self.knl, self.alpha))
    ...
    def fit(self, X, y, lr: float = None, iterations: int = None):
        ...
        for ii in range(self.iterations):
            ...
            for ij in range(ylen):
                ...
                self.alphasample = self.alphasample - self.lr * (self.knlsample.dot(Y_samp) * (
                            np.ones(X_samp.shape[0]) - self.activation(self.knlsample,
                                                                       self.alphasample)) + self.lamda * np.dot(
                    self.knlsample, self.alphasample))
